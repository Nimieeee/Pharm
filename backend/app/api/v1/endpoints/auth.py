...
async def login(
    ...
    try:
        ...
        # --- NEW: Handle verification check here instead of in service ---
        if not user.is_verified and not user.is_admin:
            # Trigger resend automatically for convenience
            try:
                await auth_service.resend_verification_code(user.email)
            except Exception as e:
                print(f"⚠️ Failed to auto-resend code: {e}")
...
@router.post("/verify/resend", status_code=status.HTTP_200_OK)
async def resend_verification(
    ...
    """
    await auth_service.resend_verification_code(request.email)
