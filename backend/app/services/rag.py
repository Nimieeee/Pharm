...
class RAGService:
    ...
    async def _generate_embedding(self, text: str) -> Optional[List[float]]:
        """Generate embedding using Mistral API (async, non-blocking)"""
        try:
            if not text.strip():
                return None
            
            if not self.mistral_api_key:
                logger.warning("Mistral API key not configured, using fallback")
                ...
                async with httpx.AsyncClient() as client:
                    ...
                    if response.status_code == 200:
                        ...
                        if result.get("data") and len(result["data"]) > 0:
                            ...
                            logger.debug(f"Generated Mistral embedding: {len(embedding)} dimensions")
                            return embedding
                        else:
                        ...
                        if attempt < max_retries - 1:
                            ...
                            logger.warning(f"Embedding rate limit (429), retrying in {delay}s (attempt {attempt + 1}/{max_retries})")
                            await asyncio.sleep(delay)  # Non-blocking sleep
                            continue
                        else:
                            logger.error(f"Embedding rate limit exceeded after {max_retries} attempts, using fallback")
                            return self._generate_fallback_embedding(text)
    ...
    async def _process_and_store_chunk(
        ...
        try:
            ...
            result = await async_db_execute(
                lambda: self.db.table("document_chunks").insert(chunk_data).execute()
            )
    ...
    async def search_similar_chunks(
        ...
        try:
            ...
            if not query_embedding:
                logger.warning("No query embedding generated, using recent chunks")
            ...
            result = await async_db_execute(
                lambda: self.db.rpc(function_name, rpc_params).execute()
            )
    ...
    async def _get_recent_chunks(
        ...
        try:
            ...
            uid = str(user_id)
            result = await async_db_execute(
                lambda: self.db.table("document_chunks").select(
    ...
    async def delete_conversation_documents(
        ...
        try:
            ...
            await async_db_execute(
                lambda: self.db.table("document_chunks").delete().eq(
