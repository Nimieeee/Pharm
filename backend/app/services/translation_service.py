...
# Bounded concurrency for translation tasks (critical for 2vCPU VPS)
_translation_semaphore = asyncio.Semaphore(3)
# Store task references to prevent GC and observe exceptions
...
class TranslationService:
    ...
    async def translate_text(self, text: str, target_language: str, source_language: str = 'en') -> Optional[str]:
        ...
        if not self.mistral_api_key:
            logger.warning("Translation skipped: No Mistral API key")
            return None
            ...
            async with httpx.AsyncClient(timeout=60.0) as client:
                ...
                if response.status_code == 200:
                    ...
                    if result.get("choices") and len(result["choices"]) > 0:
                        ...
                        logger.debug(f"Translated to {target_language}")
                        return translated
                else:
    ...
    async def queue_message_translation(
        ...
        """Queue background translation for a message (bounded concurrency)"""
        logger.debug(f"Queueing translations for message {message_id} (Source: '{source_language}')")
        
        task = asyncio.create_task(
            self._translate_and_store_message(message_id, content, source_language)
        )
        _active_translation_tasks.add(task)
        task.add_done_callback(_active_translation_tasks.discard)
    ...
    async def _translate_and_store_message(
        ...
        """Background task to translate message and store results (semaphore-bounded)"""
        async with _translation_semaphore:
            try:
                ...
                logger.debug(f"Stored translations for message {message_id}: {list(translations.keys())}")
                
            except Exception as e:
                logger.error(f"Failed to store translations for message {message_id}: {e}")
    ...
    async def queue_conversation_title_translation(
        ...
        logger.debug(f"Queueing title translations for conversation {conversation_id}")
    ...
    async def _translate_and_store_title(
        ...
        """Background task to translate title and store results (semaphore-bounded)"""
        async with _translation_semaphore:
            try:
                ...
                await async_db_execute(
                    lambda: self.db.table("conversations").update({
                        "title_translations": translations_json
                    }).eq("id", conv_id).execute()
                )
                
                logger.debug(f"Stored title translations for conversation {conversation_id}")
                
            except Exception as e:
                logger.error(f"Failed to store title translations: {e}")
    ...
    async def get_message_in_language(
        ...
        try:
            ...
            logger.error(f"Error getting message translation: {e}")
            return None
