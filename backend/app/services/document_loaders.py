...
class EnhancedDocumentLoader:
    ...
    async def _load_pdf(
        ...
        """
        Ultra-fast Hybrid PDF Loader:
        Leverages PyMuPDF (fitz) and selective VLM via app.services.vision_service
        """
        import os
        from app.services.vision_service import process_pdf_hybrid
        
        logger.info(f"🚀 Using hybrid PyMuPDF extraction for {filename}")
        
        with open(file_path, 'rb') as f:
            content = f.read()
            
        api_key = os.getenv("MISTRAL_API_KEY", "")
        extracted_text = await process_pdf_hybrid(
            content=content,
            filename=filename,
            user_prompt="",
            api_key=api_key,
        ...
        doc = Document(
            page_content=self._clean_text(extracted_text),
            metadata={"source": filename, "loader": "PyMuPDF_Hybrid"}
        )
        return [doc]
    ...
    async def _load_pptx(
        ...
        """
        Ultra-fast Hybrid PPTX Loader:
        Leverages python-pptx extraction directly instead of heavy unstructured loaders
        """
        import os
        from app.services.vision_service import process_pptx_hybrid
        
        logger.info(f"🚀 Using hybrid python-pptx extraction for {filename}")
        
        with open(file_path, 'rb') as f:
            content = f.read()
            
        api_key = os.getenv("MISTRAL_API_KEY", "")
        extracted_text = await process_pptx_hybrid(
            content=content,
            filename=filename,
            user_prompt="",
            api_key=api_key,
        ...
        doc = Document(
            page_content=self._clean_text(extracted_text),
            metadata={"source": filename, "loader": "python-pptx_Hybrid"}
        )
        return [doc]
