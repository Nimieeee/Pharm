...
async def async_db_execute(func: Callable[..., T], *args, **kwargs) -> T:
    """Run a sync Supabase operation in a thread pool to avoid blocking the event loop.
    
    Usage:
        result = await async_db_execute(
            lambda: db.table('messages').select('*').eq('id', msg_id).execute()
        )
    """
...
class Database:
    ...
    async def test_connection(self) -> bool:
        ...
        try:
            ...
            # Test with a simple query — run in thread to avoid blocking
...
async def init_db():
    ...
    try:
        ...
        try:
            await async_db_execute(
                lambda: db.get_client().table("users").select("id").limit(1).execute()
            )
            logger.info("Users table exists - migrations appear to be complete")
        except Exception:
            logger.warning("Users table not found - please run migrations")
