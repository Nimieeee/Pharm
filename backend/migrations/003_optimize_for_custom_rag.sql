-- Migration 003: Optimize database for custom RAG + Supabase pgvector integration
-- This migration optimizes the existing schema for better custom RAG implementation
...
-- Add optimized HNSW index for better performance with custom RAG
...
-- Add metadata indexes for custom RAG filtering
...
-- Optimize the match_document_chunks function for custom RAG
...
-- Create function for custom RAG similarity search with score
...
-- Add comments for custom RAG integration
COMMENT ON FUNCTION match_document_chunks_custom IS 'Custom RAG-optimized similarity search with metadata filtering';
COMMENT ON FUNCTION similarity_search_with_score IS 'Custom RAG similarity search returning distance scores';
COMMENT ON FUNCTION get_embedding_stats IS 'Get statistics about stored embeddings for monitoring';
COMMENT ON INDEX document_chunks_embedding_hnsw_idx IS 'HNSW index optimized for custom RAG similarity search';
COMMENT ON INDEX document_chunks_embedding_ivf_idx IS 'IVFFlat index for compatibility and fallback';

-- Create view for custom RAG document retrieval
...
COMMENT ON VIEW rag_documents IS 'View optimized for custom RAG document retrieval';