...
function cleanMermaidSyntax(raw: string): string {
    ...
    // CRITICAL FIX: Split concatenated nodes BEFORE line-by-line processing
    // Example: ]B --> or )B --> or }B -->
    cleanedText = cleanedText.replace(/([]\]\)\}])\s*([A-Za-z0-9_-]+)\s*(-->|--|<-->|<--|\.->|-.->|==>|==|\.\.>|\.\.)/g, '$1\n$2 $3');
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        // --- HEURISTIC 0a: Missing arrow before edge label with pipe (CRITICAL)
        line = line.replace(/([]\]\)\}]+)\s*"([^"]*)"\s*\|([A-Za-z0-9_-])/g, '$1 -->|"$2"|$3');

        // --- HEURISTIC 0b: Missing arrow before quoted edge label (no pipe)
        line = line.replace(/([]\]\)\}]+)\s*"([^"]+)"\s*([A-Za-z0-9_\[\(])/g, '$1 --> "$2" $3');

        // --- HEURISTIC 0c: Missing arrow before node (no quotes)
        ...
        // --- HEURISTIC 1A: Fix node IDs with spaces before label ---
        // AI sometimes outputs: Beta-Lactam Ring["label"] instead of BetaLactamRing["label"]
        // Pattern: Word Word["label"] → WordWord["label"]
        line = line.replace(/\b([A-Za-z][A-Za-z0-9_-]*)\s+([A-Za-z][A-Za-z0-9_-]*)\s*(\[[^\]]*\]|\([^\)]*\))/g, '$1$2$3');
